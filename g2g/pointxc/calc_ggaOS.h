...
__host__ __device__ void calc_ggaOS(scalar_type dens_a, scalar_type dens_b,
  ...
  if ((dens_a > MINIMUM_DENSITY_VALUE) || (dens_b > MINIMUM_DENSITY_VALUE)){
    if (Vxc_id == 9) {  // PBE
      ...
      scalar_type grad2_a =
          grad_a.x * grad_a.x + grad_a.y * grad_a.y + grad_a.z * grad_a.z;
      dgrad_a = sqrt(grad2_a);
      delgrad_a =
          (grad_a.x * grad_a.x * hess1_a.x +
            ...
            grad_a.y * grad_a.y * hess1_a.y + grad_a.z * grad_a.z * hess1_a.z) / dgrad_a;
      rlap_a = hess1_a.x + hess1_a.y + hess1_a.z;  // Laplacian Up

      // Down density
      scalar_type grad2_b =
          grad_b.x * grad_b.x + grad_b.y * grad_b.y + grad_b.z * grad_b.z;
      dgrad_b = sqrt(grad2_b);
      delgrad_b =
          (grad_b.x * grad_b.x * hess1_b.x +
            ...
            grad_b.y * grad_b.y * hess1_b.y + grad_b.z * grad_b.z * hess1_b.z) / dgrad_b;
      rlap_b = hess1_b.x + hess1_b.y + hess1_b.z;

      // Up + Down densities
      scalar_type grad2 = grad.x * grad.x + grad.y * grad.y + grad.z * grad.z;
      dgrad = sqrt(grad2);
      delgrad = (grad.x * grad.x * hess1.x + grad.y * grad.y * hess1.y
