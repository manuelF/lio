...
// evaluate_partition() and autotune_partition() are now in
// partition_autotune.{h,cpp}.
...
void Partition::regenerate(void) {
  ...
  if (G2G::cpu_threads > 0 && G2G::gpu_threads > 0 && do_autotune) {
    AutotuneResult at = autotune_partition(
        x0, x1, all_points, min_exps_func, min_coeff_func,
        cpu_threads, gpu_threads,
        little_cube_size, sphere_radius, sphere_decomp_size);
    if (little_cube_size < 0) little_cube_size = at.cube_size;
    if (sphere_radius < 0) sphere_radius = at.sphere_radius;
    if (sphere_decomp_size < 0) sphere_decomp_size = at.sphere_decomp;
  }
#endif
